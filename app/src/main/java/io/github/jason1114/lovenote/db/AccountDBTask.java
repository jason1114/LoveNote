...
public class AccountDBTask {
    ...
    public static OAuthActivity.DBResult addOrUpdateAccount(AccountBean account,
        ...
        cv.put(AccountTable.OAUTH_TOKEN, account.getAccessToken());
        cv.put(AccountTable.OAUTH_TOKEN_EXPIRES_TIME, String.valueOf(account.getExpiresTime()));
    ...
    public static List<AccountBean> getAccountList() {
        ...
        while (c.moveToNext()) {
            ...
            account.setAccessToken(c.getString(colid));

            colid = c.getColumnIndex(AccountTable.OAUTH_TOKEN_EXPIRES_TIME);
            account.setExpiresTime(Long.valueOf(c.getString(colid)));
